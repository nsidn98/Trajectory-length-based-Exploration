...
class DDQN_Agent:
    def __init__(self, env, save_name, args):
        ...
        if self.args.CNN:
            self.current_model = CnnDDQN(self.env.observation_space.shape, self.env.action_space.n).to(device)
            self.target_model = CnnDDQN(self.env.observation_space.shape, self.env.action_space.n).to(device)
            self.buffer_size = 100000
            self.replay_initial = 10000
        else:    
            self.current_model = DDQN(self.env.observation_space.shape[0], self.env.action_space.n).to(device)
            self.target_model = DDQN(self.env.observation_space.shape[0], self.env.action_space.n).to(device)
